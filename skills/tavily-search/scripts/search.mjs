...
// 优先从轮换池获取 Key，如果没有则使用环境变量
let apiKey = "";
try {
  apiKey = execSync('python3 /root/.openclaw/workspace/tavily-pool.py get', { encoding: 'utf-8' }).trim();
...
if (!apiKey) {
  console.error("Missing TAVILY_API_KEY (轮换池和环境变量都不可用)");
  process.exit(1);
}
...
// 增加使用计数（如果使用轮换池）
try {
  execSync('python3 /root/.openclaw/workspace/tavily-pool.py increment', { encoding: 'utf-8' });
} catch (e) {
  // 忽略计数错误
}
